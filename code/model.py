...
def train_cbow_model(training_data):
    ...
    model = Word2Vec(sentences = training_data, alpha= 0.06,min_count=1, ns_exponent=10e-3, window=4, sg=0, hs=1, size=100,iter=10)
...
def train_cbow_model_line_sentence(training_data):
    print("Training started.....")
    model = Word2Vec(corpus_file = training_data, alpha= 0.06, min_count=1, ns_exponent=10e-3, window=4, sg=0, hs=1,size=100,iter=10)
    model.wv.save_word2vec_format(EMBEDDINGS)
    print("Training done")

def main():
    if parse_args().format == 'true':
        training_data = return_gz_format('/Users/oluwayetty1/Desktop/School/1-2/NLP/public_homework2/data/training.txt')
